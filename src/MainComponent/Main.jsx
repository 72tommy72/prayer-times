...
export default function Main() {
    ...
    //         const fujrToMidnightDiff = nextPrayerTime.diff(
    ...
    if (
        ...
        // If current time is after Isha, reset to Fajr of the next day
        prayerIndex = 0;
    }
    setNextPrayer(prayerIndex);

    const nextPrayerObj = prayerList[prayerIndex].name; // Use prayerIndex
    const nextPrayerTime = aladanApi[nextPrayerObj];

    // Create a moment object from nextPrayerTime
    const nextPrayerTimeMoment = moment(nextPrayerTime, "HH:mm");

    // If current time is after the next prayer time, add a day to the next prayer time
    if (momentNow.isAfter(nextPrayerTimeMoment)) {
        nextPrayerTimeMoment.add(1, 'days');
    }
                    ...
                    <div className="row my-4 py-3 text-white d-flex justify-content-around ms-auto me-auto inner-card">
                        <div className="col-md-5 my-2 ">
                            <div className="py-4 t-right border border-2 border-solid border-white">
                                <p>متبقى حتى صلاة {prayerList[nextPrayer].displayName + " "}</p>
                                <h2>{remaningTime}</h2>
                            </div>
                        </div>
                        <div className="col-md-5  my-2">
